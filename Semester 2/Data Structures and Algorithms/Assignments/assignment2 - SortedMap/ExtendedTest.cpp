...
void testAddIfNotPresentIncreasing()
{
	cout << "Test add if not present" << endl;
	SortedMap originalSortedMap(increasing);
	SortedMap sm(increasing);

	originalSortedMap.add(1, 3);
	originalSortedMap.add(4, 5);
	originalSortedMap.add(7, 3);

	sm.add(1, 8);
	sm.add(3, 7);
	sm.add(2, 8);

	assert(originalSortedMap.addIfNotPresent(sm) == 2);
	assert(originalSortedMap.size() == 5);
	assert(originalSortedMap.search(3) != NULL_TVALUE);
	assert(originalSortedMap.search(2) != NULL_TVALUE);

}

void testAddIfNotPresentDecreasing()
{
	cout << "Test add if not present" << endl;
	SortedMap originalSortedMap(decreasing);
...
void testAllExtended() {
	...
	testIterator();
	testQuantity();
	testAddIfNotPresentIncreasing();
	testAddIfNotPresentDecreasing();
}
