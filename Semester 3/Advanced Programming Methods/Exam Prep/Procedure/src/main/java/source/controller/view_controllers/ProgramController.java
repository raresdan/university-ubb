...
public class ProgramController {
    ...
    @FXML
    private TableView<Map.Entry<String, javafx.util.Pair<List<String>, IStatement>>> procedureTable;

    @FXML
    private TableColumn<Map.Entry<String, javafx.util.Pair<List<String>, IStatement>>, javafx.util.Pair<String, List<String>>> signatureColumn;

    @FXML
    private TableColumn<Map.Entry<String, javafx.util.Pair<List<String>, IStatement>>, String> bodyColumn;
    ...
    private void initialize()
    {
        ...
        signatureColumn.setCellValueFactory(p -> new SimpleObjectProperty<>(new javafx.util.Pair<>(p.getValue().getKey(), p.getValue().getValue().getKey())));
        bodyColumn.setCellValueFactory(p -> new SimpleStringProperty(p.getValue().getValue().toString()));
    }
    ...
    public void setProgramStatement(IStatement programStatement)
    {
        ...
        Interpreter.addProcedures(procTable);
        PrgState currentProgram = new PrgState(programStatement, procTable);
    ...
    private void populateSymbolTableView()
    {
        ...
        if (currentProgram != null)
            for (Map.Entry<String, IValue> entry : currentProgram.getSymbolTableTop().getContent().entrySet())
    ...
    private void populateProcTable() {
        ...
        if (state != null) {
            for (Map.Entry<String, javafx.util.Pair<List<String>, IStatement>> entry : state.getProcedureTable().getContent().entrySet()) {
                procTableList.add(entry);
            }
        }
        procedureTable.setItems(FXCollections.observableList(procTableList));
