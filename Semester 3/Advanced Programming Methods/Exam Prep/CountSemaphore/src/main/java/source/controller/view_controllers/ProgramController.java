...
public class ProgramController {
    ...
    @FXML
    private TableColumn<Map.Entry<Integer, javafx.util.Pair<Integer, List<Integer>>>, Integer> semaphoreIndexColumn;
    ...
    private void initialize()
    {
        ...
        semaphoreListColumn.setCellValueFactory(p -> new SimpleObjectProperty<>(p.getValue().getValue().getValue()));
    }
    ...
    private void populateSemaphoreTable() {
        ...
        List<Map.Entry<Integer, javafx.util.Pair<Integer, List<Integer>>>> semaphoreTableList = new ArrayList<>();
        if (state != null) {
            semaphoreTableList.addAll(state.getSemaphoreTable().getContent().entrySet());
        }
